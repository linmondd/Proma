...
比如我安装的 Gmail MCP，核心就是利用 Gmail 的 API 来帮我收发邮件；也可以安装文档、日程或项目管理类 MCP，让 Proma Agent 修改或创建外部服务里的信息。现在整个互联网上也已经存在了相当大量的 MCP 以及关于这些 MCP 的教程等信息，所以它天然适合直接通过 Proma Agent  自己为自己安装 MCP 了。



#### 如何安装  MCP？

首先还是分为两种情况，一种是你知道大概的 MCP 是什么，这取决于你是如何设计你的工作的，就像上文一样，当你用着 Proma Agent 时突发奇想为什么不能直接编辑某个文档服务呢？于是才产生了对对应 MCP 的需求，此时就可以直接跟 Proma Agent 在当前工作区对话帮你搜索是否有合适 MCP 进行安装，全程自然语言对话即可完成，完成后重启 Proma 即可生效。
...
这样就可以完成对应的 MCP 的安装了。通常这类 MCP 都会涉及到一定的授权，比如 Gmail 或文档服务都需要你来生成一些授权信息，同样可以跟着 Proma Agent 的引导即可完成。
